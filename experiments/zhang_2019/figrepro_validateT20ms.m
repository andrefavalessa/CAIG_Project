function v=figrepro_validateT20ms(cfg,shots,fog,fogSim,probabilities,latent,principal,d,control,referenceWindow)
...
assert(isequal(probabilities.Properties.VariableNames, ...
    {'EffectiveSec','FirstPulseSec','LastPulseSec','AvailableSec','P1','P2'}),'Truth leaked into observations.');
...
v.signAliasProbabilityResidual=max(abs((1-cos(-latent.phaseLoop1Rad))/2-probabilities.P1),[],'all');
v.fringeAliasProbabilityResidual=max(abs((1-cos(latent.phaseLoop2Rad+2*pi))/2-probabilities.P2),[],'all');
...
offGrid=shots(1,:); offGrid.FirstPulseSec=offGrid.FirstPulseSec+1e-6;
offGrid.LastPulseSec=offGrid.LastPulseSec+1e-6;
v.offGridWindowRejected=rejects(@() figrepro_windowMean(fog,offGrid,cfg.fog.sampleRateHz),'figrepro:OffGridWindow');
early=principal(1,:); early.AvailableSec=early.LastPulseSec-.001;
v.futureSupportRejected=rejects(@() figrepro_synchronizeT20ms(fog,early,cfg.fog),'figrepro:FutureData');
assert(v.offGridWindowRejected && v.futureSupportRejected);
v.control=struct;
if ~isempty(control)
    c=control.caig; e=control.estimate; s=control.synchronized;
    ...
    altered=probabilities(1,:); altered.P1(1,1)=altered.P1(1,1)+1e-5;
    labels=control.externalBranchLabels;
    names={'sign1','sign2','fringe1','fringe2'};
    for j=1:numel(names), labels.(names{j})=labels.(names{j})(1,:); end
    ...
    v.control.causalPrefixMaxStateDifference=max(abs(prefixEstimate.state-e.state(1:cut,:)),[],'all');
    assert(v.control.causalPrefixMaxStateDifference==0,'Future samples changed past control estimates.');
    ...
    changedP=probabilities; changedP.P1(cut+1:end,:)=.4; changedP.P2(cut+1:end,:)=.6;
    recon=figrepro_knownBranchControl(changedP,cfg.inverse,control.externalBranchLabels);
    v.control.probabilityPrefixDifference=max(abs(recon.RateARadPerSec(1:cut,:)-c.RateARadPerSec(1:cut,:)),[],'all');
    ...
    v.control.finalMisalignmentErrorDeg=v.control.finalMisalignmentDeg-rad2deg(cfg.simulation.misalignmentRad).';
    v.control.finalBiasErrorDegH=v.control.finalBiasDegH-rad2deg(cfg.simulation.biasFRadPerSec)*3600;
    ...
    if strcmp(cfg.caseName,'constant_speed')
        ...
        U=e.sensitivityNullspace; D=diag(1./cfg.filter.priorStd);
        v.control.nullspacePriorCovarianceChange=norm(U.'*D*e.covariance(:,:,end)*D*U-eye(3),'fro');
        assert(v.control.nullspacePriorCovarianceChange<1e-7);
        v.control.qualitativeInterpretation='Figure 6 style: rank 3/6; prior-dependent state plateaus, even with oracle branch labels.';
    else
        assert(e.sensitivityRank==6);
        v.control.qualitativeInterpretation='Figure 8 style control: rank 6/6; richer excitation. State errors retain model/noise effects; oracle branch labels are required.';
    end
...
try
    ...
    if strcmp(issue.identifier,identifier), yes=true; else, rethrow(issue); end
end
end
