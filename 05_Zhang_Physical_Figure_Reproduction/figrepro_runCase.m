...
priorPath=path; guard=onCleanup(@() path(priorPath));
folders={'config','motion','sensors','synchronization','estimation','plotting'};
for k=1:numel(folders), addpath(fullfile(projectRoot,folders{k})); end
...
effective=start+cfg.caig.Tsec; last=start+2*cfg.caig.Tsec;
...
result.cfg=cfg; result.fog=fog; result.fogSimulation=fogSimulation;
result.probabilities=probabilities; result.latentCAIG=latentCAIG;
result.caig=caig; result.synchronized=synced; result.estimate=estimate;
