...
% P is the state covariance; it is not an atomic transition probability.
n = height(synced);
x = settings.initialState;
P = diag(settings.priorStd.^2);

estimate.state = zeros(n, 6);
estimate.std = zeros(n, 6);
estimate.innovation = zeros(n, 3);
estimate.covariance = zeros(6, 6, n);
estimate.information = zeros(6);
estimate.Hstack = zeros(3 * n, 6);

for j = 1:n
    % Prediction: the configured F and Q retain constant calibration states.
    x = settings.F * x;
    P = settings.F * P * settings.F.' + settings.Q;

    % Build the first-order Zhang measurement model from reconstructed rate:
    % z = omega_F - omega_A ~= [omega_A]_x * phi + b.
    % phi denotes mounting misalignment, not interferometer phase.
    a = synced.RateARadPerSec(j,:);
    S = [0 -a(3) a(2);a(3) 0 -a(1);-a(2) a(1) 0];
    H = [S eye(3)];
    z = (synced.RateFRadPerSec(j,:) - a).';

    % Measurement covariance and innovation, with ideal CAIG readout.
    ...
    % Joseph-form state covariance update improves numerical robustness.
    J = eye(6) - K * H;
    P = J * P * J.' + K * R * K.';
    P = (P + P.') / 2;

    % Store causal estimates and diagnostics; no truth enters this loop.
    estimate.state(j,:) = x.';
