...
priorPath=path; pathGuard=onCleanup(@() path(priorPath));
folders={'config','motion','sensors','synchronization','estimation','plotting'};
for j=1:numel(folders), addpath(fullfile(projectRoot,folders{j})); end
cfg=figrepro_configT20ms(caseName);
if nargin<3, out=[]; end
if nargin<4, baselineFile=''; end
...
effective=start+T; last=start+2*T; available=last+cfg.caig.readoutSec;
...
comparison.T1msMaxAbsDifferentialPhaseRad=max(abs(baseline.result.latentCAIG.phaseDiffRad),[],'all');
...
result.cfg=cfg; result.probabilities=probabilities; result.latentCAIG=latent;
result.fog=fog; result.fogSimulation=fogSimulation;
...
result.diagnostics=diagnostics; result.validation=validation;
...
summary=diagnostics;
summary.validation=validation; summary.comparisonWithPreserved1ms=comparison;
...
if ~isempty(control)
    fprintf('KNOWN-BRANCH CONTROL ONLY: rank %d/6; alignment [deg] %.6f %.6f %.6f; bias [deg/h] %.6f %.6f %.6f.\n', ...
        control.estimate.sensitivityRank,validation.control.finalMisalignmentDeg,validation.control.finalBiasDegH);
end
fprintf('20 ms validation passed; saved outputs with prefix %s.\n',cfg.outputStem);
end

function writeJson(file,value)
fid=fopen(file,'w'); assert(fid>=0,'Cannot write 20 ms summary.');
