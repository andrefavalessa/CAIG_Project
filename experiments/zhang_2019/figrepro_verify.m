...
% Validate the physical phase chain only after measurement-based estimation.
...
v.rateReconstructionMaxErrorRadPerSec = max(abs(caig.RateARadPerSec - latent.omegaARadPerSec), ...
    [], 'all');
assert(v.differentialPhysicsMaxErrorRad < 1e-11 && v.phaseInverseMaxErrorRad < 1e-11 && ...
    v.rateReconstructionMaxErrorRadPerSec < 1e-12, 'figrepro:PhysicsCheck', ...
        'Physical chain identity failed.');
...
altered.P1(1, 1) = altered.P1(1, 1) + 1e-4;
...
assert(abs(v.probabilityInterventionRateChange) > 1e-8, 'figrepro:ProbabilityBypass', ...
    'Probability intervention did not reach rate.');
...
assert(v.causalPrefixMaxStateDifference == 0, 'figrepro:Causality', ...
    'Future data changed an earlier estimate.');
tooEarly = caig(1,:);
tooEarly.AvailableSec = tooEarly.LastPulseSec;
v.unavailableSupportRejected = false;
try
    figrepro_synchronize(fog, tooEarly, fogSim.sampleSigmaRadPerSec);
catch issue
    if strcmp(issue.identifier, 'figrepro:FutureData'),
        v.unavailableSupportRejected = true;
    else,
        rethrow(issue);
    end
end
assert(v.unavailableSupportRejected, 'figrepro:Causality', ...
    'Unavailable interpolation support was accepted.');
...
% State covariance must remain symmetric and numerically positive semidefinite.
...
assert(v.minimumCovarianceEigenvalue >= -1e-18 && all(isfinite(estimate.state), 'all'), ...
    'Invalid filter state/covariance.');
...
% Excitation determines identifiable state combinations, not just convergence.
if strcmp(cfg.caseName, 'constant_speed')
    ...
    assert(v.nullspacePriorCovarianceChange < 1e-7, ...
        'Unobservable directions falsely gained information.');
    v.qualitativeInterpretation = ...
        'Figure 6 style weak observability: only three state combinations are identifiable; individual plateaus depend on the prior.';
else
    assert(v.sensitivityRank == 6, 'Expected sway sensitivity rank 6.');
    v.qualitativeInterpretation = ...
        'Figure 8 style rich excitation: all six sensitivity directions present; inspect saved estimates for convergence and residual model/noise error.';
end
