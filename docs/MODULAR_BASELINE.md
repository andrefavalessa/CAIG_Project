...
CAIG_Project/
  ...
  run_zhang_experiment.m
  src/{config,motion,sensors,synchronization,estimation,validation,visualization,utils}/
  experiments/zhang_2019/
  experiments/legacy_analysis/
  archive/{v1,v2,figures}/
  docs/
  references/
~~~~
...
| `src/estimation/runZhangKalman.m` | Six-state Zhang model using MATLAB `trackingKF` |
| `src/validation/validateMainSimulation.m` | Phase, synchronization, noise-level, and Kalman regression checks |
| `src/visualization/printSimulationSummary.m` | Grouped terminal report and overall acceptance result |
...
The main estimator keeps the Zhang state and measurement equations but executes them through `trackingKF`. The dedicated manual-Kalman validation script remains unchanged in `experiments/legacy_analysis` as independent evidence.
...
Detailed notes are kept in `references/parameter_sources.md`.
