...
% Audit each physical loop before judging the principal reconstructed rate.
L1 = latent.phaseLoop1Rad;
L2 = latent.phaseLoop2Rad;
outside1 = L1 < 0 | L1 > pi;
outside2 = L2 < 0 | L2 > pi;

% Simulator-only sign/fringe labels; these are not inferred from P1/P2.
wrapped1 = atan2(sin(L1), cos(L1));
wrapped2 = atan2(sin(L2), cos(L2));
controlLabels.sign1 = ones(size(L1));
controlLabels.sign1(wrapped1 < 0) = -1;
controlLabels.sign2 = ones(size(L2));
controlLabels.sign2(wrapped2 < 0) = -1;
...
% Scale, timing and phase magnitudes for the configured experiment.
d.caseName = cfg.caseName;
d.shotCount = size(L1, 1);
d.scalarLoopPhaseCount = 2 * numel(L1);
d.KsingleSec = cfg.caig.KsingleSec;
d.KdualSec = cfg.caig.KdualSec;
...
% Keep scalar-loop, per-axis and whole-shot violation counts distinct.
...
% Nonzero wrapped fringe labels need not imply an absolute phase above 2*pi.
...
% Compare the candidate against latent rate only on this diagnostic boundary.
