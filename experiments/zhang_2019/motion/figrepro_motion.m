...
if strcmp(caseName, 'constant_speed')
    ...
    lat = s.latitudeRad;
    h = s.heightM;
    heading = s.headingRad;

    % Local curvature radii from the configured ellipsoid.
    ...
    % Platform velocity components, distinct from CAIG atomic velocity.
    vN = s.speedMPerSec * cos(heading);
    vE = s.speedMPerSec * sin(heading);

    % Earth rate plus transport rate, resolved in the local NED frame.
    omegaIN_N = [s.earthRateRadPerSec*cos(lat);0;-s.earthRateRadPerSec*sin(lat)] + ...
        [vE/(rn+h);-vN/(rm+h);-vE*tan(lat)/(rn+h)];

    % Rotate the frozen navigation-frame rate into the CAIG A frame.
    ...
    a = s.swayAmplitudeRad;
    w = 2 * pi ./ s.swayPeriodSec;

    roll = a(1) * sin(w(1) * t);
    pitch = a(2) * sin(w(2) * t);
    rollDot = a(1) * w(1) * cos(w(1) * t);
    pitchDot = a(2) * w(2) * cos(w(2) * t);
    headingDot = a(3) * w(3) * cos(w(3) * t);

    % Body rates include Euler-angle coupling, not just angle derivatives.
