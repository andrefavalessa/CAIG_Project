...
% Retain the declared rate envelope without imposing a principal phase branch.
...
for axisIndex = 1:3
    ...
    % Counterpropagating atomic velocities reverse the rotation contribution.
    v1 = p.atomSpeedMPerSec * p.v1DirectionsA(axisIndex,:);
    v2 = -v1;

    % Coriolis/Sagnac rotation phases from the same Zhang vector model.
    ...
    % Ideal static gravity compensation; no physical feedback servo.
...
% Retain both total phases so the later branch audit can inspect each loop.
...
% Differential phase remains a latent validation quantity, not an observation.
...
% Return timing and transition probabilities, even when phases are ambiguous.
measurement = shots;
